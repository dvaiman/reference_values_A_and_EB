...
dfc <- df %>% 
  mutate(
    Astrand_rel_VO2 = Astrand_MaxVO2 / WeightKG * 1000,
         ...
         Astrand_rel_VO2 = case_when(Astrand_rel_VO2 < 15 | Astrand_rel_VO2 > 90 ~ NA_real_,
         ...
         EkB_rel_VO2 = case_when(EkB_rel_VO2 < 15 & Gender == "Male" ~ NA_real_,# 24
                                 EkB_rel_VO2 > 90 & Gender == "Male" ~ NA_real_, # 76
                                 EkB_rel_VO2 < 15 & Gender == "Female" ~ NA_real_, #"19"
                                 EkB_rel_VO2 > 90 & Gender == "Female" ~ NA_real_, #62
                                 TRUE ~ EkB_rel_VO2),
...
mean = quantile({{ variable }}, c(.1, .25, .50, .75, .9)),

percentile = c(.1, .25, .50, .75, .9)
...
plot_smooth_data <- function(df, variable, xlim = NULL, ylim = NULL, title = NULL) {
  ...
  ggplot(smooth_data, aes(Age, {{ variable }}, group = percentile, color = as.character(percentile))) +
    ...
    geom_text(data = text_data,
              aes(x = 13, y = minvo2,
                  label = paste0(percentile * 25, "%")),
              color = "black",
              hjust = 0,
              show.legend = FALSE) +
    facet_wrap(~ Gender) +
    #scale_color_viridis_d(option = "rocket") +
    scale_color_manual(values = c("#291535", "#2f6b8e", "#291535")) +
    theme_minimal(
        base_size = 14,
  base_family = "Roboto",
    ) +
...
plot_smooth_data <- function(df, variable, xlim = NULL, ylim = NULL, title = NULL) {
  smooth_data <- df %>%
    ...
    group_by(Gender, Age) %>%
    mutate(
      percentile = cut({{ variable }}, quantile({{ variable }}, c(0, 0.1, 0.25, 0.5, 0.75, 0.9, 1), na.rm = TRUE), include.lowest = TRUE, labels = FALSE),
      label_percentile = cut({{ variable }}, quantile({{ variable }}, c(0, 0.1, 0.25, 0.5, 0.75, 0.9, 1), na.rm = TRUE), include.lowest = TRUE, labels = c("10%", "25%", "50%", "75%", "90%"))
    ) %>%
    ungroup() %>%
    drop_na(percentile, Gender, Age)
  ...
  text_data <- smooth_data %>%
    ...
    summarise(max_age = max(Age),
              min_age = min(Age),
              minvo2 = mean({{ variable }}),
              label_percentile = first(label_percentile)) %>%
    ungroup()
  ...
  ggplot(smooth_data, aes(Age, {{ variable }}, group = percentile, color = as.character(percentile))) +
    ...
    geom_text(data = text_data,
              ...
              show.legend = FALSE) +
    facet_wrap(~ Gender) +
    scale_color_manual(values = c("#291535", "#2f6b8e", "#291535", "#2f6b8e", "#291535", "#2f6b8e", "#291535")) +
...
plot_smooth_data(dfc, Astrand_rel_VO2, xlim = c(10, 65), ylim = c(10,90), title = "Åstrand mL")
