...
df <- read_csv2(here::here("fitness reference values ekblom and astrand", "data", "2014-2022.csv"))
...
dfc %>%  
  ...
  reframe(mean = quantile(EkB_rel_VO2, c(.1,.25,.40,.50,.60,.75,.9)), 
...
#  ungroup() %>% 
  ...
  pivot_wider( id_cols = c(Gender, agegroup_5, n),
               ...
               values_from = mean,
               names_prefix = "q") %>% 
  mutate( Age = glue::glue("{agegroup_5} yrs"),
          VeryLow = glue::glue("<{sprintf('%.1f', q10)}"),
          Low = glue::glue(">{sprintf('%.1f', q10)}-{sprintf('%.1f', q25)}"),
          SomewhatLow = glue::glue(">{sprintf('%.1f', q25)}-{sprintf('%.1f', q40)}"),
          Average1 = glue::glue(">{sprintf('%.1f', q40)}-"),
          Average2 = glue::glue("{sprintf('%.1f', q50)}"),
          Average3 = glue::glue("-{sprintf('%.1f', q60)}"),
          SomewhatHigh = glue::glue(">{sprintf('%.1f', q60)}-{sprintf('%.1f', q75)}"),
          High = glue::glue(">{sprintf('%.1f', q75)}-{sprintf('%.1f', q90)}"),
          VeryHigh = glue::glue(">{sprintf('%.1f', q90)}")
          ) %>% 
...
dfc %>%  
  ...
  drop_na(EkB_MaxVO2, agegroup_5) %>% 
  group_by(Gender, agegroup_5) %>% 
  reframe(mean = quantile(EkB_MaxVO2, c(.1,.25,.40,.50,.60,.75,.9)), 
            percentile = c(.1,.25,.40,.50,.60,.75,.9),
            n=n()) %>% 
#  ungroup() %>% 
  mutate(mean = round(mean,1),
...
dfc %>%  
  ...
  drop_na(Astrand_rel_VO2, agegroup_5) %>% 
  group_by(Gender, agegroup_5) %>% 
  reframe(mean = quantile(Astrand_rel_VO2, c(.1,.25,.40,.50,.60,.75,.9)), 
            percentile = c(.1,.25,.40,.50,.60,.75,.9),
            n=n()) %>% 
#  ungroup() %>% 
  mutate(mean = round(mean,1),
...
dfc %>%  
  ...
  drop_na(Astrand_MaxVO2, agegroup_5) %>% 
  group_by(Gender, agegroup_5) %>% 
  reframe(mean = quantile(Astrand_MaxVO2, c(.1,.25,.40,.50,.60,.75,.9)), 
            percentile = c(.1,.25,.40,.50,.60,.75,.9),
            n=n()) %>% 
#  ungroup() %>% 
  mutate(mean = round(mean,1),
...
dfc %>%  
  ...
  drop_na(Astrand_TO2, agegroup_5) %>% 
  group_by(Gender, agegroup_5) %>% 
  reframe(mean = quantile(Astrand_TO2, c(.1,.25,.40,.50,.60,.75,.9)), 
            percentile = c(.1,.25,.40,.50,.60,.75,.9),
            n=n()) %>% 
#  ungroup() %>% 
  mutate(mean = round(mean,1),
...
smooth_data <- dfc %>%
  filter(DiagHighBloodPressure == "N",
         ...
         Age <= 65,
         Gender != "Other") |>
  #drop_na(EkB_rel_VO2, Age) %>%
  group_by(Gender, Age) %>%
  mutate(percentile = ntile(EkB_rel_VO2, 10),
         percentileA = ntile(Astrand_rel_VO2, 10)) |>
  ungroup() |>
...
text_data <- smooth_data %>%
  ...
  summarise(max_age = max(Age),
            min_age = min(Age),
            minvo2 = mean(EkB_rel_VO2),
            minvo2A = mean(Astrand_rel_VO2)) %>%
  ungroup()

ggplot(smooth_data, aes(Age, EkB_rel_VO2, group = percentile, color = as.character(percentile))) +
  ...
  theme_minimal() +
  labs(title = "EB") +
  xlim(c(10, 65)) + # Adjust the x-axis limits max(text_data$max_age+10))
...
smooth_data <- dfc %>%
  ...
  #drop_na(EkB_rel_VO2, Age) %>%
  group_by(Gender, Age) %>%
  mutate(percentile = ntile(EkB_MaxVO2, 10),
         percentileA = ntile(Astrand_rel_VO2, 10)) |>
  ungroup() |>
  drop_na(percentile, Gender, Age)
...
text_data <- smooth_data %>%
  ...
  summarise(max_age = max(Age),
            min_age = min(Age),
            minvo2 = mean(EkB_MaxVO2),
            minvo2A = mean(Astrand_rel_VO2)) %>%
  ungroup()

ggplot(smooth_data, aes(Age, EkB_MaxVO2, group = percentile, color = as.character(percentile))) +
  geom_point(alpha=0.1, shape=21
             )+
  geom_smooth(
    aes( group = percentile, ),
...
smooth_data <- dfc %>%
  ...
  mutate(percentile = ntile(EkB_rel_VO2, 10),
         percentileA = ntile(Astrand_rel_VO2, 10)) |>
  ungroup() |>
  drop_na(percentileA, Gender, Age)
...
ggplot(smooth_data, aes(Age, Astrand_rel_VO2, group = percentileA, color = as.character(percentileA))) +
  geom_point(alpha=0.1, shape=21
             )+
  geom_smooth(
    aes( group = percentileA ),
    color= "gray30",
    se = TRUE,
    fullrange = FALSE,
    span = 0.9
...
smooth_data <- dfc %>%
  ...
  group_by(Gender, Age) %>%
  mutate(percentile = ntile(EkB_rel_VO2, 10),
         percentileA = ntile(Astrand_MaxVO2, 10)) |>
  ungroup() |>
  drop_na(percentileA, Gender, Age)
...
text_data <- smooth_data %>%
  ...
  summarise(max_age = max(Age),
            min_age = min(Age),
            minvo2 = mean(EkB_rel_VO2),
            minvo2A = mean(Astrand_MaxVO2)) %>%
  ungroup()

ggplot(smooth_data, aes(Age, Astrand_MaxVO2, group = percentileA, color = as.character(percentileA))) +
  geom_point(alpha=0.1, shape=21
             )+
  geom_smooth(
    aes( group = percentileA ),
