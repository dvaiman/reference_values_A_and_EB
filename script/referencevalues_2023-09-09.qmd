...
# model values R2 lm between vo2 and exercise answer when adjusted for Age
exercise <-
dfc_long %>%
  group_by(test, Gender) %>%
  group_modify(~ broom::glance(lm(vo2 ~ ExerciseAnswer + Age, data = .x))) |> 
  mutate(AgeGroup="50-59 years")
...
dfc_long |> 
  ggplot(aes(Age, vo2, group = ExerciseAnswer, color =ExerciseAnswer )) +
  geom_smooth(method = lm, se = FALSE) +
  facet_wrap(~ Gender + test)
...
# geom_smooth model values

dfc_long %>%
  group_by(test, Gender) %>%
  group_map(~ summary(mgcv::gam(vo2 ~ s(Age, bs = "cs"), data = .x))) %>%
  map_dbl(~ xxx)


dfc_long %>% nest(-c(test, Gender)) %>% 
  mutate(fit = map(data, ~mgcv::gam(vo2 ~ s(Age, bs = "cs"), data = .)),
...
# graph with smooth
dfc_long |> 
  ggplot(aes(Age, vo2, group = as.character(ExerciseAnswer), color = as.character(ExerciseAnswer) )) +
...
# model data
BMI <- 
dfc_long_BMI %>% nest(-c(test, Gender, AgeGroup)) %>% 
  drop_na(AgeGroup) |> 
...
print(BMI |> mutate(R.square=round(R.square,4)) |> select( Gender, AgeGroup, test, R.square)) |> arrange(desc(R.square)) |> 
  print(n=50)
