...
dfc |> 
  track(.headline = "Flow chart: EB-test   ",
...
dfc |> 
  track(.headline = "Å-test flow chart ",
       # .messages="n={.count}"
        ) %>%
 # status() %>%

    filter(Astrand_rel_VO2 > 0, .messages="Excluded n={formatC(.excluded, big.mark = ',')} without the Å-test" )|>  # between(Age, 20,69)
    #comment(.headline = "") |>  
    group_by(Gender, .messages="Stratify by sex\nn={formatC(.count, big.mark = ',')}") |>

  # does not deal with NA without na.rm true
exclude_all(
       #Gender == "Other" ~ "Exclude {.excluded} sex other" ,
       Age < 20 | Age > 69 ~ "Excluded n={formatC(.excluded, big.mark = ',')} with age < 20 or age > 69" ,
       DiagHighBloodPressure == "Y" ~ "Excluded n={formatC(.excluded, big.mark = ',',  digits = 6)} with High Blood Pressure",
       HeartMedicine != "N" ~ "Excluded n={formatC(.excluded, big.mark = ',')} with Heart Medicine",
       PulseMedicine != "N" ~ "Excluded n={formatC(.excluded, big.mark = ',')} with Pulse Medicine",
       TobaccoSmoking < 5 ~ "Excluded n={formatC(.excluded, big.mark = ',',  digits = 6)} Smokers", 
       na.rm = TRUE,
       .headline = "{Gender}"
  ) |>  comment(.headline = "\n{Gender} ", 
                "n={formatC(.count, big.mark = ',')}") |> 
...
formatC(22000, big.mark = ',',  digits = 6)
```
...
df_filtered <- dfc |>
  ...
  pivot_longer(cols = c(EkB_rel_VO2, Astrand_rel_VO2),
               names_to = "type",
               values_to = "vo2") |> 
  mutate(type = if_else(type=="EkB_rel_VO2", "E-B", "Astr")) |> 
  drop_na(vo2)
...
df_filtered |>
 ...
 # scale_point_color_gradient(low = "#132B43", high = "#56B1F7") +
  ...
  facet_grid(Gender ~ factor(type, levels=c('E-B', 'Astr'))) +
...
dfc_long %>%
  ...
  group_modify(~ broom::glance(lm(vo2 ~ ExerciseAnswer, data = .x))) |> 
...
dfc_long_BMI |> 
 ...
 # scale_y_continuous(limits = c(25,65)) +
    #scale_color_viridis_d(option="rocket", end = 0.9, direction = 1) +
  scale_color_manual(values = (c("#AE445A", "#662549","black","#213555", "#4F709C"))) +
  
        labs(
    x = "BMI",  
    y = expression("Estimated vo"[2]*"max"),  
