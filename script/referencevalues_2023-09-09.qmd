...
testtype <-
dfc |> 
  mutate(test=case_when(EkB_rel_VO2 >= 1 & Astrand_rel_VO2 >= 1 ~ "Both",
                   is.na(EkB_rel_VO2) & Astrand_rel_VO2 >= 1 ~ "Å-test",
                   EkB_rel_VO2 >= 1 & is.na(Astrand_rel_VO2) ~ "EB-test",
                   TRUE~NA)) |>
  track(.headline = "Flow chart: test type     ",
        .messages="Data from 2010-2022"
  ) %>%
  filter(!is.na(test), .headline = "Missing data on test type    ",
         .messages = "Excluded {.excluded}") |> 
  group_by(test, 
           .headline = "",
           .messages="Stratify by test\nn={formatC(.count, big.mark = ',')}"
           
  ) |>
  status(.headline = "{test}") |> 
  # does not deal with NA without na.rm true
  exclude_all(
    # Gender == "Other" ~ "Exclude {.excluded} sex other" ,
    Age < 20 | Age > 69 ~          "{formatC(.excluded, big.mark = ',',format = 'fg')} with age < 20 or age > 69" ,
    DiagHighBloodPressure != "N" ~ "{formatC(.excluded, big.mark = ',',format = 'fg')} with High Blood Pressure",
    HeartMedicine != "N" ~         "{formatC(.excluded, big.mark = ',',format = 'fg')} with Heart Medicine",
    PulseMedicine != "N" ~         "{formatC(.excluded, big.mark = ',',format = 'fg')} with Pulse Medicine",
    TobaccoSmoking < 5 ~           "{formatC(.excluded, big.mark = ',',format = 'fg')} Smokers", 
    na.rm = TRUE,
    .headline = "{test} excluded"
  ) |>  
  comment(.headline = "", 
                "n={formatC(.count, big.mark = ',')}") |> 
  ungroup(.headline = "Final sample "#, .messages="n={.count}"
  ) 
...
eb <-
dfc |> 
  track(.headline = "Flow chart: the EB-test   ",
        .messages=" Data from 2010-2022"
  ) %>%
  group_by(Gender, 
           .headline = "",
           .messages="Stratify by sex\nn={formatC(.count, big.mark = ',')}"
           
  ) |>
  status(.headline = "{Gender}") |> 
  filter(EkB_rel_VO2 > 0, 
         .messages=" {formatC(.excluded, big.mark = ',')} without the EB-test",
         .headline="{Gender} excluded" )|>  # between(Age, 20,69)
...
# render plot, a4 width is 8.3 inches
eb |> 
  flowchart(filename="flowcharteb",
            formats = c("png","svg", "pdf",
                         maxWidth=30, 
                        maxHeight=30))
...
atest<-
dfc |> 
  track(.headline = "Flow chart: the Å-test   ",
        .messages=" Data from 2010-2022"
  ) %>%
  group_by(Gender, 
           .headline = "",
...
atest |> 
  flowchart(filename="flowchartatest",
            formats = c("png","svg", "pdf"),
                         maxWidth=20, 
                        maxHeight =20)
...
# significance and effect size tests

```{r}

# statistically compare the eb test between men and women

lm(EkB_rel_VO2 ~ Gender, data=eb |>  mutate(Gender=if_else(Gender == "Male",1,0))) |> broom::tidy()

lm(Astrand_rel_VO2 ~ Gender -1, data=atest) |> broom::tidy()|> 
  mutate(round(p.value,3))


# Cohens D
atest %>% rstatix::cohens_d(Astrand_rel_VO2 ~ Gender, var.equal = TRUE)

eb %>% rstatix::cohens_d(EkB_rel_VO2 ~ Gender, var.equal = TRUE)

atest %>% rstatix::cohens_d(Astrand_MaxVO2 ~ Gender, var.equal = TRUE)

eb %>% rstatix::cohens_d(EkB_MaxVO2 ~ Gender, var.equal = TRUE)
...
# ((first value-last value) / first value) / number of decades
...
bind_cols(
  ...
  eb |> 
    ...
    mutate(
      ...
      BMI_ekb = sprintf("%.1f +%.1f", round(BMI_mean, 1), round(BMI_sd, 1)), # "%.1f ±%.1f" removed +-
 ...
 # mutate(AgeGroup = fct_relevel(agegroup_5, c("20-29 years", "30-39 years", "40-49 years", "50-59 years", "60-69 years", "+70 years"))) %>%
  ...
  mutate(
   ...
   mutate(across(c(WeightKG_ekb, WeightKG_Astrand, HeightCM_ekb, HeightCM_Astrand, BMI_ekb, BMI_Astrand,  Exercise_ekb, Exercise_Astrand),
                as.character))
...
openxlsx::write.xlsx(table1, here::here("tables" , "table1.xlsx"))
...
df_filtered <- dfc |>
  ...
  select(EkB_rel_VO2, Astrand_rel_VO2, AgeGroup, Gender) |> 
  pivot_longer(cols = c(EkB_rel_VO2, Astrand_rel_VO2),
               names_to = "type",
               values_to = "vo2") |> 
  mutate(type = if_else(type=="EkB_rel_VO2", "EB-test", "Å-test")) |> 
  drop_na(vo2)

df_summary <- df_filtered %>%
  group_by(AgeGroup, type, Gender) %>%
  summarize(vo2_max = max(vo2, na.rm = T))

# df_filtered |>
#   ggplot(aes(x = vo2, y = AgeGroup, fill = after_stat(x))) +
#   geom_density_ridges_gradient(scale = 3, size = 0.3, rel_min_height = 0.0001) +
#     geom_text(
#     data = df_summary,
#     aes(y = AgeGroup, label = AgeGroup, x = vo2_max),
#     hjust = .1,
#     vjust = 0,
#     size = 8,
#     fill = "white",
...
df_filtered |>
  ggplot(aes(x = vo2, y = AgeGroup)) +
  stat_density_ridges(
    aes( 
      fill = factor(after_stat(quantile))
      
      ),
    jittered_points = TRUE, position = "raincloud", point_size = 0.2, point_alpha = .02,
     scale = 1,
    geom = "density_ridges_gradient",
    calc_ecdf = TRUE,
    quantiles = c(0.10,.25,.4,.6,.75,.90), 
    point_color = "grey10",
    color="white"
  ) +
 # scale_point_color_gradient(low = "#132B43", high = "#56B1F7") +
    geom_text(
    data = df_summary,
    aes(y = AgeGroup, label = AgeGroup, x = 70),
    family = "Roboto",
    hjust = .1,
    vjust = -0.5,
    size = 6,
    ...
    show.legend = FALSE
      ...
      colorspace::darken("#DA9694", 0.2), 
      colorspace::darken("#E6B8B7", 0.13), 
      colorspace::darken("#F2DCDB", 0.05),
      colorspace::darken("#ECECEC", 0),
      colorspace::darken("#DAEEF3", 0.05),
      colorspace::darken("#B7DEE8", 0.13),
      colorspace::darken("#92CDDC", 0.2))) +
  facet_grid(Gender ~ factor(type, levels=c('EB-test', 'Å-test'))) +
  ...
  labs(
    x = expression("Estimated vo"[2]*"max (mL/min/kg)"),  
    y = "", 
  ...
  scale_x_continuous(limits = c(15,110), breaks = c(20,40,60,80) ) +
      ...
      strip.text = element_text(size = 16, face = "bold", color = "grey30"), 
    plot.background = element_rect(color = "white", fill = "white"),
     panel.background = element_rect(color = "white", fill = "white"),
...
ggsave(width = 8, height = 8, dpi = 600, "referenceridges.png")
ggsave(width = 8, height = 8, dpi = 600, "referenceridges.svg")

```

# -------------------------------

# -Reference Table EB Astrand 
...
```{r}
  ...
  ggplot(smooth_data |> mutate(vo2=round(vo2,0)) |> filter(Age < 66), aes(Age, vo2, group = percentile_group, color = as.character(percentile_group))) +
    geom_point(alpha = 0.3, shape = 16, size = 2) + #alpha = 0.08, shape = 16, size = 2.2
    ...
    scale_color_manual(values = c( 
      colorspace::darken("#DA9694", 0.3), 
      colorspace::darken("#E6B8B7", 0.18), 
      colorspace::darken("#F2DCDB", 0.1),
      colorspace::darken("#ECECEC", 0),
      colorspace::darken("#DAEEF3", 0.1),
      colorspace::darken("#B7DEE8", 0.18),
      colorspace::darken("#92CDDC", 0.3))) +
    ...
    theme(
      ...
      strip.text = element_text(size = 18, face = "bold", color = "grey30"), # Increase facet label size and font weight
    plot.background = element_rect(color = "white", fill = "white"),
     panel.background = element_rect(color = "white", fill = "white")
    )
  
  ggsave("refplot_mL_pink.png", width = 11, height = 8, dpi=600)
...
```{r}
  ...
  ggplot(smooth_data |> mutate(vo2=round(vo2,1)) |> filter(Age < 66), aes(Age, vo2, group = percentile_group, color = as.character(percentile_group))) +
    geom_point(alpha = 0.3, shape = 16, size = 2) + #alpha = 0.08, shape = 16, size = 2.2
    geom_text(data = text_data %>% filter(Gender == unique(Gender)[1]), # Filter based on the first facet
              aes(x = 19, y = minvo2,
                  label = label_percentile,
...
ggsave("refplot_L_pink.png", width = 11, height = 8, dpi=600)
...
dfc_long <- 
  ...
  select(ExerciseAnswer, Gender, AgeGroup, Age, Astrand_rel_VO2, EkB_rel_VO2) |> 
  group_by(ExerciseAnswer, Gender, Age, AgeGroup) |> 
  pivot_longer(cols = c(EkB_rel_VO2, Astrand_rel_VO2),
               names_to = "test",
               values_to = "vo2") |> 
  ungroup() |> 
  mutate(
    test = factor(test, levels= c("EkB_rel_VO2", "Astrand_rel_VO2")),
    ExerciseAnswer = case_when(
      ExerciseAnswer == 1 ~ "Never",
      ExerciseAnswer == 2 ~ "Sometimes",
      ExerciseAnswer == 3 ~ "1–2\ntimes/\nweek",
      ExerciseAnswer == 4 ~ "3–5\ntimes/\nweek",
      ExerciseAnswer == 5 ~ "At least\n6 times/\nweek",
      TRUE ~ NA_character_  # Handle other cases (if any)
    ),
    ExerciseAnswer = factor(ExerciseAnswer, levels = c(
      "Never", "Sometimes", "1–2\ntimes/\nweek", "3–5\ntimes/\nweek", "At least\n6 times/\nweek"
    )),
    AgeGroup = case_when( 
      AgeGroup == "20-29 years" ~ "20-29", 
      AgeGroup == "30-39 years" ~ "30-39",
      AgeGroup == "40-49 years" ~ "40-49",
      AgeGroup == "50-59 years" ~ "50-59",
      AgeGroup == "60-69 years" ~ "60-69",
      TRUE ~ NA_character_ 
    ),
  ...
  drop_na(vo2, Age, ExerciseAnswer) 
```
## R2 for graphs

```{r}

# model values R2 lm between vo2 and exercise answer
exercise <-
dfc_long %>%
  group_by(test, Gender) %>%
  group_modify(~ broom::glance(lm(vo2 ~ ExerciseAnswer + Age, data = .x))) 

```

## R2 for exercise and vo2 by agegroup

```{r}

dfc_long %>%
  group_by(test, Gender, AgeGroup) %>%
  group_modify(~ broom::glance(lm(vo2 ~ as.character(ExerciseAnswer), data = .x))) |> 
  select(test, Gender, AgeGroup, r.squared, adj.r.squared ) |> 
   arrange(desc(r.squared)) |>
  drop_na(AgeGroup) |> 
  print(n=50) |> arrange(r.squared)
...
dfc_long %>%
  group_by(test) %>%
  group_modify(~ broom::glance(lm(vo2 ~ as.character(ExerciseAnswer) + Gender + Age, data = .x))) |> 
  select(test,  r.squared, adj.r.squared ) |> 
   arrange(desc(r.squared)) 
...
dfc_long %>%
  ...
  select(test,  r.squared, adj.r.squared ) |> 
   arrange(desc(r.squared)) 
# A model adjusted for sex, age and self reported exercise was explaining 43% and 25% of the variance for the EB and Å-test respectively, while in a model with only self reported exercise the explained variance was 8% and 10%.
...
# overall
 ...
 # group_by(test, Gender, AgeGroup) %>%
  lm(vo2 ~ as.character(ExerciseAnswer), data = dfc_long) |> broom::tidy()


dfc_long %>%
  group_by(test, Gender, AgeGroup) %>%
  group_modify(~ broom::tidy(lm(vo2 ~ as.character(ExerciseAnswer), data = .x))) |> print(n=120)
```
...
dfc_long |> 
    filter(AgeGroup!="+70 years") |> 
  ggplot(aes(ExerciseAnswer, vo2, group = AgeGroup, color = AgeGroup )) +
  geom_smooth(method = lm, se = FALSE) +
  geom_text(data = exercise, aes(x=1.5, y=48, label = paste( "R2 =",round(r.squared,2) )), color="grey30") +
  facet_grid(Gender~ test, labeller = labeller(test = c("EkB_rel_VO2" = "E-B", "Astrand_rel_VO2" = "Astr")))  + 
  scale_x_discrete(guide = guide_axis(n.dodge = 1)) +
  #scale_color_viridis_d(name = "", option = "magma") +
  scico::scale_color_scico_d(direction = -1, palette = "acton", end = .85) +
  ...
  theme(legend.position = "top",
    ...
    labs(
    x = "",  # Empty x-axis title
    y = expression("Estimated vo"[2]*"max"),  # Y-axis title with subscript 2
    color = ""  # Remove color legend title
  )
...
dfc_long |> 
    filter(AgeGroup!="+70 years") |> 
  ggplot(aes(ExerciseAnswer, vo2 )) +

  #  geom_violin(aes(ExerciseAnswer, vo2, fill = AgeGroup ), position = position_dodge(1)) + 
    geom_boxplot(aes( AgeGroup , vo2, fill =  ExerciseAnswer), position = position_dodge(1),   outlier.alpha = 0) + 

  facet_grid(Gender ~ test, labeller = labeller(test = c("EkB_rel_VO2" = "E-B", "Astrand_rel_VO2" = "Astr")))  +
    scico::scale_fill_scico_d(direction = -1, palette = "tokyo", end = 1) +
  theme_minimal() +
  theme(legend.position = "top",
            axis.text = element_text(size = 12),
    axis.title = element_text(size = 14, color = "grey30"),
...
dfc_long |> 
    filter(AgeGroup!="+70 years") |> 
  ggplot(aes(AgeGroup, vo2 , fill =  ExerciseAnswer)) +
ggdist::stat_interval(.width = c(.25, .5, .95), size = 4,  # c(.25, .5, .95, 1)
    size = 7, 
    position = position_dodge(1)) +
  stat_summary(
    geom = "point", fun = median,
    color = "white", size = 2, shape = 23, stroke = 1, position = position_dodge(1)
  ) +
  #   ggdist::stat_halfeye(
  #   adjust = .33, ## bandwidth
  #   width = .7, fill = "grey85",
...
dfc_long %>%
  filter(AgeGroup != "+70 years") %>%
  ggplot(aes(AgeGroup, vo2, color = ExerciseAnswer)) +
  ggdist::stat_pointinterval(position = position_dodge(1),
                             .width = c(0.66, .95) #c(0.5, 0.95)
                             ) +
  facet_grid(Gender ~ test, labeller = labeller(test = c("EkB_rel_VO2" = "EB-test", "Astrand_rel_VO2" = "Å-test"))) +
  # scale_color_viridis_d(
  #   option = "mako", name = "", direction = 1,
  #   begin = .15, end = .9
  # ) +
   #scico::scale_color_scico_d(direction = 1, palette = "tofino", begin = .1, end = .9) +
 #  scale_color_manual(values = (c("steelblue", "steelblue4","black","lightpink4", "lightpink3"))) +
scale_color_manual(values = (c("#A2678A", "#4D3C77","black","#5C8374", "#93B1A6"))) +
  ...
  theme(
    ...
    axis.text = element_text(size = 12),
    axis.title.y = element_text(size = 14, color = "grey30", margin = margin(r = 10)), 
    axis.title.x = element_text(size = 14, color = "grey30", margin = margin(t = 10)), 
    strip.text = element_text(size = 16, face = "bold", color = "grey30"),
    panel.grid.minor = element_blank(),
    panel.grid.major.x = element_blank(),
    plot.background = element_rect(color = "white", fill = "white"),
  ...
  labs(
    x = "Age group (years)",
    y = expression("Estimated vo"[2] * "max (mL/min/kg)"),
    color = "Exercise"
  )


ggsave(width = 9, height = 6, dpi = 600, "vo2vsexercise.png")
ggsave(width = 9, height = 6, dpi = 600, "vo2vsexercise.svg")
...
dfc |> 
  ...
  filter(Age >= 20 , Age <= 70) |> 
  select(BMI, Gender, AgeGroup, Age, Astrand_rel_VO2, EkB_rel_VO2) |> 
  group_by(BMI, Gender, Age) |> 
  pivot_longer(cols = c(EkB_rel_VO2, Astrand_rel_VO2),
               names_to = "test",
               values_to = "vo2") |> 
  ungroup() |> 
  drop_na(vo2, AgeGroup, BMI) |> 
   mutate(
    test = factor(test, levels= c("EkB_rel_VO2", "Astrand_rel_VO2")),
    AgeGroup = case_when( 
      AgeGroup == "20-29 years" ~ "20-29", 
...
## R2 for BMI and vo2 by agegroup
```{r}
# model data

BMI <- 
dfc_long_BMI %>% nest(data = -c(test, Gender, AgeGroup)) %>% 
  drop_na(AgeGroup) |> 
  mutate(fit = map(data, ~mgcv::gam(vo2 ~ s(BMI, bs = "cs", k = 5), data = .)),
         results = map(fit, broom::glance),
         R.square = map_dbl(fit, ~ summary(.)$r.sq)) %>%
  unnest(results) %>%
  ungroup() |> 
  arrange(R.square) |> 
  select(-data, -fit, Gender, AgeGroup,test,R.square) 

print(BMI |> mutate(R.square=round(R.square,4)) |> 
        select( Gender, AgeGroup, test, R.square)) |> 
  arrange(desc(R.square)) |> 
  print(n=50)

# BMI + age + gender
dfc_long_BMI %>% nest(data = -c(test)) %>% 
 # drop_na(AgeGroup) |> 
  mutate(fit = map(data, ~mgcv::gam(vo2 ~ s(BMI, bs = "cs", k = 5) + Gender+ Age, data = .)),
         results = map(fit, broom::glance),
         R.square = map_dbl(fit, ~ summary(.)$r.sq)) %>%
  unnest(results) %>%
  ungroup() |> 
  arrange(R.square) |> 
  select(-data, -fit,test,R.square) 

# A model adjusted for sex, age and BMI was explaining 67% and 34% of the variance for the EB and Å-test respectively, while in a model with only BMI the explained variance was 29% and 23%.
...
dfc_long_BMI %>%
  group_by(test, Gender, AgeGroup) %>%
  group_modify(~ broom::glance(lm(vo2 ~ BMI, data = .x))) |> 
  select(test, Gender, AgeGroup, r.squared, adj.r.squared ) |> 
   arrange(desc(r.squared)) |>
  print(n=50) |> 
  drop_na(AgeGroup)
...
# plot
dfc_long_BMI |> 
  filter(AgeGroup!="+70 years", BMI <=50 ) |> 
  ggplot(aes(x=BMI, y=vo2, group = AgeGroup, color = AgeGroup )) +
  #geom_density() +
  geom_smooth(method = NULL, se = FALSE) +
  #geom_rug(alpha=0.1) +
  #geom_point(color = "transparent") +
  # geom_text(data = BMI, aes(x=40, y=50, label = paste( "R2 =",round(R.square,2) )), color="grey30") +
   facet_grid(Gender ~ test, labeller = labeller(test = c("EkB_rel_VO2" = "EB-test", "Astrand_rel_VO2" = "Å-test"))) +
 # scale_y_continuous(limits = c(25,65)) +
    # bscale_color_viridis_d(option="rocket", end = 0.9, direction = 1) +
 # scale_color_manual(values = (c("#AE445b", "#662549","black","#213555", "#4F709C"))) +
 #scale_color_manual(values = c( "#c5b0d5", "#9467bd","#333333","#ff7f0e", "#ffbb78")) +
 #scale_color_manual(values = c("#6e67c7", "#ffd700","black", "#333333", "#ff6600"))+
#scale_color_manual(values = c("#005689", "#ff7300", "#333333", "#8b8b8b", "#1f6e93"))

scale_color_manual(values = c("#005689", "#3F6366", "#7F7144", "#BF7E22", "#FF8C00"))+
        labs(
    x = expression(BMI~(kg/m^2)),  
     y = expression("Estimated vo"[2]*"max (mL/min/kg)"), 
    color = "Age group"  # Remove color legend title
  )+
  theme_minimal() +
  theme(
    legend.position = "top",
...
ggsave(width = 9, height = 6, dpi = 600, "vo2VSbmi.png")
ggsave(width = 9, height = 6, dpi = 600, "vo2VSbmi.svg")
...
# Generate 5 evenly spaced colors in the gradient
gradient_colors <- colorRampPalette(c(start_color, end_color))(5)
